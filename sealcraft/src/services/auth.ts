...
import { dbGet, dbSet, dbDelete, dbQuery, dbSetEncrypted, dbGetEncrypted, generateId } from '@/utils/db'
...
function getCurrentUserId(): string {
  const authData = dbGetEncrypted<{ userId?: string; token?: string }>(AUTH_COLLECTION, 'current')
...
export async function loginWithHarmony(): Promise<{ userInfo: User; token: string }> {
  const privacyAgreed = uni.getStorageSync('sealcraft_privacyAgreed') as boolean | undefined
  if (!privacyAgreed) {
    ...
    uni.login({
      ...
      success: (res) => {
        if (res.code) {
          ...
          dbSetEncrypted(AUTH_COLLECTION, 'current', { userId, token })
...
export async function deleteAccount(): Promise<void> {
  ...
  // 删除用户相关的收藏
  const collections = dbQuery<{ collectionId: string }>('collections', (c) => (c as any).userId === userId)
  collections.forEach((c) => dbDelete('collections', (c as any).collectionId))
