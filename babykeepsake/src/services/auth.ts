...
function getCurrentUserId(): string {
  const authData = uni.getStorageSync('babykeepsake_auth') as { userId?: string } | undefined
...
export async function loginWithHarmony(): Promise<{ userInfo: User; token: string }> {
  const privacyAgreed = uni.getStorageSync('babykeepsake_privacyAgreed') as boolean | undefined
  if (!privacyAgreed) {
...
export async function deleteAccount(): Promise<void> {
  ...
  // 删除用户相关的里程碑
  const milestones = dbQuery<{ milestoneId: string }>('milestones', (m) => (m as any).userId === userId)
  milestones.forEach((m) => dbDelete('milestones', (m as any).milestoneId))
