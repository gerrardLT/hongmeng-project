 ...
 * 格式化工具函数
 */
import type { AquariumType, MaintenanceType, AlertLevel } from '@/types/models'
...
/**
 * 格式化参数值带单位
 * @param value 参数值，null 时显示 "--"
 * @param unit 单位
 */
export function formatParamValue(value: number | null, unit: string): string {
 ...
 * 水族箱类型中文名
 */
export function formatAquariumType(type: AquariumType): string {
  const map: Record<AquariumType, string> = {
    freshwater: '淡水缸',
    planted: '草缸',
    marine: '海水缸',
    shrimp: '虾缸'
