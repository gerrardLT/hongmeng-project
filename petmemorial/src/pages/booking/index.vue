      ...
      <view v-if="filteredList.length > 0" class="list-content">
        ...
        >
          <view class="item-header">
            <text class="memorial-name">{{ getMemorialName(item.typeId) }}</text>
          ...
          <view class="item-body">
            ...
            <view class="info-row">
              <text class="info-icon">📅</text>
              <text class="info-text">{{ item.date }} {{ item.timeSlot }}</text>
            </view>
          </view>
...
// 缓存名称
const memorialNameMap = ref<Record<string, string>>({})
...
function getMemorialName(typeId: string): string {
  if (memorialNameMap.value[typeId]) return memorialNameMap.value[typeId]
  const detail = getMemorialTypeById(typeId)
  if (detail) {
    memorialNameMap.value[typeId] = detail.name
    return detail.name
  }
  return '纪念品'
}
