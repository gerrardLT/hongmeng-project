      ...
      <view class="info-card">
        <view class="info-card-header">
          ...
          <view class="info-content">
            <text class="info-memorial-name">{{ memorialName }}</text>
            <text class="info-type-label">{{ typeLabel }}</text>
          </view>
...
const currentStageName = computed(() => {
  const current = progressList.value.find((p) => p.status === 'in-progress')
...
function loadData(bookingId: string) {
  // 加载进度
  const progresses = getProgressByBookingId(bookingId)
  ...
  if (booking) {
    ...
    if (memorial) {
      ...
      typeLabel.value = memorial.duration
      categoryIcon.value = CATEGORY_ICONS[memorial.category] || '🎁'
    }
