...
import { getMemorialTypes, getAllCategories } from '@/services/memorial'
import type { MemorialType, MemorialCategory } from '@/types/models'

const currentCategory = ref<MemorialCategory | 'all'>('all')
const memorialList = ref<MemorialType[]>([])

// 分类 Tab 数据
const categoryTabs = [
  { label: '全部', value: 'all' as const },
  ...getAllCategories().map((c) => ({ label: c.label, value: c.value as MemorialCategory | 'all' }))
]

// 根据分类筛选
const filteredList = computed(() => {
  if (currentCategory.value === 'all') return memorialList.value
  return memorialList.value.filter((m) => m.category === currentCategory.value)
})

function loadData() {
  const list = getMemorialTypes()
  memorialList.value = list
}
...
// Tab 切换
function onTabChange(value: MemorialCategory | 'all') {
